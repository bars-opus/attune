...
import 'package:attune/features/planning/presentation/screens/create_planning_task_screen.dart';
...
// planningTasksProvider (invalidated via .notifier.refresh() after a
...
void main() {
  testWidgets('an unsaved draft survives a failed save, and offers retry', (
    ...
    final gateway = _FakeGateway({
      'create_planning_task': (params) {
        attempts++;
        throw StateError('network down');
      },
    });
    await tester.pumpWidget(_wrap(
      const CreatePlanningTaskScreen(relationshipId: 'r1'),
      PlanningRepository(gateway),
    ));
    await tester.pumpAndSettle();

    await tester.enterText(find.byType(TextField).first, 'Book the venue');
    await tester.pump();
    ...
    expect(find.textContaining('Could not create the task'), findsOneWidget);
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(find.textContaining('Could not create the task'), findsNothing);

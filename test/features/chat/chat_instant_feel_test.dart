...
void main() {
  ...
  test('sent message appears immediately and stays at the top through ack',
      () async {
    ...
    expect(
      state.messages.where((m) => m.content == 'brand new').length,
      1,
    );
  });

  test('a server timestamp slightly behind local time does not reorder',
      () async {
    final repo = FakeChatRepository(currentUserId: userId);
    final container = buildChatContainer(repository: repo, userId: userId);
    addTearDown(container.dispose);

...
void main() {
  testWidgets(
    'renders the thumbnail with a play button, no video controller until tap',
    (tester) async {
  ...
  testWidgets(
    'starting video playback clears a currently-playing voice message (cross-media pause)',
    (tester) async {
      final container = ProviderContainer();
      addTearDown(container.dispose);
      container.read(currentlyPlayingVoiceMessageIdProvider.notifier).state =
          'voice-1';
      ...
      //
      // Several explicit pumps rather than one: the failure path now has more
      // than one async hop before it rolls back — a failed REMOTE url is
      // retried once against a freshly re-signed URL (onRequestFreshUrl)
      // before the widget gives up, since a signed URL that expired while the
      // chat sat open is the single most common real-world cause of this
      // failure. pumpAndSettle can't be used here: this test's videoUrl is a
      // fake remote host, so the platform's own network attempt never fully
      // resolves under the test binding and settling would time out.
      for (var i = 0; i < 5; i++) {
  ...
  testWidgets(
    'identity is keyed on clientMessageId-equivalent messageId, stable across a widget rebuild with a new videoUrl',
    (tester) async {
  ...
  testWidgets(
    'local-vs-remote source: a videoUrl not starting with http uses a local file source',
    (tester) async {

...
The `key: ValueKey(_currentModule.id)` on `TabsWithContent` is required, not optional — without it, swapping `_currentModule` would leave Flutter's tab controller in place (wrong tab count if the related tab's presence differs, and potentially the wrong `initialIndex` state carried over from the previous module). Keying on the module id forces a clean remount whenever the displayed module changes.
...
- Widget test: confirm the `TabsWithContent` key change means switching modules doesn't crash or throw a "tab count changed" assertion — this is the specific risk the `ValueKey(_currentModule.id)` addition exists to prevent, so a test that actually drills in and back is the regression guard for it, not just a visual check.

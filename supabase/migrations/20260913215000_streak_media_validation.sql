-- Widen the media-insert trigger to accept 'streak'.
--
-- This is the THIRD place media_type is gated, after the two CHECK
-- constraints in 20260913210000. The spec identified only two; this one
-- surfaced when the contract test still failed with 'Unsupported chat
-- media type' after both constraints were widened. Exactly the drift
-- class that hid the voice-note outage, one layer deeper.
--
-- Everything else is reproduced verbatim from the live definition: the
-- intent lookup, the storage-object checks, the per-type size ceilings,
-- the intent consumption, and the video thumbnail branch.
...
BEGIN
  ...
  v_max_size := CASE NEW.media_type
    ...
    -- whole-streak one.
    WHEN 'streak' THEN 12582912 -- 12MB
